...
func Get_mappingdatabase(codeagen string) (string, string, string, string, string) {
	...
	tbl_trx_transaksimember := `"db_wigo_` + strings.ToLower(codeagen) + `".tbl_trx_transaksimember_12digit`
	view_invoice_client := `"db_wigo_` + strings.ToLower(codeagen) + `".view_invoice_client`

	return tbl_mst_config, tbl_trx_transaksi, tbl_trx_transaksidetail, tbl_trx_transaksimember, view_invoice_client
}
